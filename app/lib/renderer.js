...
function paintPeopleCard(buf, cx, cy, card, idx) {
  ...
  bStr(buf, cx + 1, cy + 4, `  ${block.padEnd(inner - 3)}`, A.smoke, cardBg);

  // Row 5: Loyalty label
  const loy = card.loyalty != null ? card.loyalty : '?';
  const loyStr = `  Loy: ${loy}`;
  bStr(buf, cx + 1, cy + 5, loyStr.padEnd(inner), A.smoke, cardBg);
...
function paintMoveCard(buf, cx, cy, card, idx) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    bStr(buf, cx + 1, cy + 2 + i, `  ${text}`, i === 0 ? A.chalk : A.smoke, cardBg);
  }
